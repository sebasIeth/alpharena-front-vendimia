...
export default function RegisterPage() {
  ...
  const handleWalletRegister = async () => {
    ...
    try {
      ...
      const { nonce, message } = await api.getWalletRegisterNonce(address);

      // 2. Sign the nonce message (ECDSA, returns 0x-prefixed hex)
      const signature = await signMessageAsync({ message });

      // 3. Register with nonce + signature
      const data = await api.registerWithWallet(address, signature, nonce);
      login(data.user);
      router.push("/dashboard");
    } catch (err) {
